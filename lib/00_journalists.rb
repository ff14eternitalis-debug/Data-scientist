...
questions = [
  ...
  "3. Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas) ?",
  "4. Combien commencent par une majuscule (première lettre juste après le @) ?",
  "5. Trie la liste de handle par ordre alphabétique",
  "6. Trie la liste de handle par taille (les plus petits en premiers)",
  "7. Quelle est la position dans l'array de @epenser ?",
...
when 1
  puts "Il y a #{handles.length} handles de journalistes"
when 2
  puts "Le plus court handle est : #{handle_plus_court} (#{handle_plus_court.length - 1} caractères sans le @)"
when 3
  puts "Il y en a #{handles_5_chars.length}"
  puts "Liste : #{handles_5_chars.join(', ')}"
when 4
  puts "Il y en a #{handles_majuscule.length}"
when 5
  puts handles_alphabetique.join(', ')
when 6
  puts handles_par_taille.join(', ')
when 7
  puts "Le handle @epenser est à la position : #{position_epenser} (index) / #{position_epenser + 1} (position)"
when 8
